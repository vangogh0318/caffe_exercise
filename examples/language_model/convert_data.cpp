...
void convert_dataset(const char* image_filename, const char* label_filename,
  ...
  printf( "[%s][%d]data magic=%d\n", __FILE__, __LINE__, magic );
  CHECK_EQ(magic, 2051) << "Incorrect image file magic.";

  label_file.read(reinterpret_cast<char*>(&magic), 4);
  printf( "[%s][%d]label magic=%d\n", __FILE__, __LINE__, magic );
  CHECK_EQ(magic, 2049) << "Incorrect label file magic.";

  image_file.read(reinterpret_cast<char*>(&num_items), 4);
  printf( "[%s][%d]data num_items=%d\n", __FILE__, __LINE__, num_items );
  label_file.read(reinterpret_cast<char*>(&num_labels), 4);
  printf( "[%s][%d]label num_labels=%d\n", __FILE__, __LINE__, num_labels );
  CHECK_EQ(num_items, num_labels);

  image_file.read(reinterpret_cast<char*>(&rows), 4);
  printf( "[%s][%d]data rows=%d\n", __FILE__, __LINE__, rows );
  ...
  // Storing to db
  int label;
  int dim_features = rows * cols;
  float* pixels = new float[dim_features];
  int count = 0;
  string value;
  ...
  for (int item_id = 0; item_id < num_items; ++item_id) {
    memset(pixels, 0x00, dim_features * sizeof(float));
    image_file.read((char *)pixels, dim_features * sizeof(float));

//    for(int i=0; i<rows; i++){
//      for(int j=0; j<cols; j++){
//        printf( "%.5f ", pixels[j+i*cols] );
//      }
//      printf( "\n" );
//    }
